{{-- resources/views/productos/index-by-almacen.blade.php --}}
@extends('layouts.app')
...
<div class="container">
    ...
    <div class="card shadow-sm">
        <div class="card-header">
             <p class="mb-0">Mostrando productos con existencia en <strong>{{ $almacen->nombre }}</strong> ({{ $almacen->ubicacion ?? 'Sin ubicación' }})</p>
        </div>
        <div class="card-body">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td class="text-center">
                                    {{-- Mostramos la cantidad del PIVOT --}}
                                    {{-- que corresponde a la relación entre $almacen y $producto --}}
                                    <span class="badge fs-6 rounded-pill {{ ($producto->pivot->cantidad ?? 0) > 0 ? 'bg-primary' : 'bg-warning text-dark' }}">
                                        {{ $producto->pivot->cantidad ?? 0 }}
                                    </span>
                                </td>
                                <td class="text-end">
                                    {{-- Las siguientes acciones siguen operando sobre el producto --}}
                                    <a href="{{ route('productos.show', $producto->id) }}" class="btn btn-sm btn-info" title="Ver Detalles del Producto"><i class="fas fa-eye"></i></a>
    ...
    <div class="mt-4">
        <a href="{{ route('productos.index') }}" class="btn btn-secondary">
         ...
         {{-- Enlace para volver a la lista de almacenes --}}
